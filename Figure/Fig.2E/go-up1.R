BP<-read.table("D:/go-used.txt",head=T,stringsAsFactors = F,sep="\t",quote="");
BP_pvalue <- (-log10(as.numeric(BP[,7])))
names(BP_pvalue)<-BP[,3]

BP_pvalue<-BP_pvalue[1:5];     #####从data_pvalue前50个功能中挑选、合并选出10个function data_pvalue.txt
...
barplot(rev(data_pvalue),horiz=T,las=1,cex.names=1,xlim=c(0,4),space=0.3,col="Dodgerblue",xlab="-log10(pvalue)",xaxt="n", border="white");
axis(1,at=c(0,2,4),lwd=3,tcl=-1);
abline(v=c(0,2,4),col="white",lwd=3);
